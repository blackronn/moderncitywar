...
param([int]$TimeoutSec = 120, [string]$Scenario = 'war', [int]$Clients = 0)
...
if ($Clients -gt 0) { $clientCount = $Clients }
...
$hostP = Start-Process -FilePath $bin `
    -ArgumentList @('--headless', '--path', $root, '--', '--smoke-host', '--speed=8', "--scenario=$Scenario", "--clients=$clientCount") `
...
if ($timedOut) {
    ...
    foreach ($c in $clientProcs) { Stop-Process -Id $c.Proc.Id -Force -ErrorAction SilentlyContinue }
    Write-Host 'SMOKE TIMEOUT'
    Get-Content $hostLog | Write-Host
    foreach ($c in $clientProcs) { if (Test-Path $c.Log) { Get-Content $c.Log | Write-Host } }
    exit 1
}
...
# SCRIPT ERROR = release build'de cokme demektir; debug yutuyor diye gecmesin
$scriptErr = $false
foreach ($ef in (Get-ChildItem $logDir -Filter '*.err' -ErrorAction SilentlyContinue)) {
    $eTxt = Get-Content $ef.FullName -Raw -ErrorAction SilentlyContinue
    if ($eTxt -match 'SCRIPT ERROR') {
        $scriptErr = $true
        Write-Host ("SCRIPT ERROR bulundu: {0}" -f $ef.Name)
    }
}
if ($scriptErr) { $pass = $false }
